/*
 * EAP-TEAPV2 server (draft-ietf-emu-teapv2)
	...
	struct wpabuf *buf, struct eap_teapv2_data *data) 
{
	...
	/*
	 * draft-ietf-emu-teapv2 Section 3.4:
	...
	/* EMSK Compound MAC field is always zero in this profile. */
	os_memset(cb->emsk_compound_mac, 0, sizeof(cb->emsk_compound_mac));
	...
	size_t bind_len)
{
	...
	/* draft-ietf-emu-teapv2 Section 3.4: only Flags = 2 is defined. */
					 ...
					 struct wpabuf *in_data)
{
	...
	if (tlv.nak) {
		...
		if (!data->inner_method_done && !data->skipped_inner_auth &&
			...
			wpa_printf(MSG_DEBUG,
				   "EAP-TEAPV2: Unexpected Crypto-Binding TLV before successful inner EAP method completion (auth=%d basic_auth_not_done=%d inner_method_done=%d skipped_inner_auth=%d)",
				   sm->cfg->eap_teapv2_auth,
				   data->basic_auth_not_done,
